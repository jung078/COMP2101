...
while [[ $# -gt 0 ]]; do
    case $1 in
        -v) verbose="yes" # set verbose mode if -v option is provided
        ;;
        -h|--help) display_help;;
        *)  if [[ -z $interface ]]; then # if it's not -v or -h, check if interface is not set
                interface=$1
            else
...
if [ "$verbose" = "yes" ]; then
    echo "Identifying default route"
fi
default_router_info=$(ip r s default)
...
if [ "$verbose" = "yes" ]; then
    echo "Checking for external IP address and hostname"
fi
# finding external information relies on curl being installed and relies on live internet connection
external_address=$(curl -s checkip.amazonaws.com)
